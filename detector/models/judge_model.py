...
class VisionClient:
    ...
    async def judge_violation(
        ...
        将每个象限图 + 其裁剪出的交通灯图 + 嫌疑车辆图逐张发送给多模态模型，
        让模型根据时间顺序判断车辆是否闯红灯。

        Args:
            quadrant_images: 象限名 → 象限图片路径，如 {"左上": "/path/左上.jpg", ...}。
            crop_images: 象限名 → 该象限裁剪出的交通灯图片路径。
        ...
        prompt = (
            ...
            "（按时间从早到晚排列），每张全景图后附有从该图中裁剪出的交通灯特写。"
            "最后一张是嫌疑车辆图。\n\n"
            "重要提示：\n"
            "- 请重点参考交通灯特写图来判断信号灯状态，全景图中的信号灯可能不清晰，以特写图为准。\n"
            "- 全景图中可能存在其他车辆的违规行为，但这与当前嫌疑车辆无关，请仅针对嫌疑车辆进行判断。\n"
            "- 请独立判断嫌疑车辆是否闯红灯，不要被图片中其他车辆的违法行为干扰。\n"
            "- 只判断是否闯红灯，压线、违停等其他违法行为不在判定范围内。\n\n"
            "请按以下步骤分析：\n"
            "1. 逐张分析每张交通灯特写图，描述该时刻信号灯的状态（红灯/绿灯/黄灯）。\n"
